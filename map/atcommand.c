	...
	const char* command, const char* message)
{
	...
	if (s_class == 0)
		up_level -= 40;
	if ((s_class == 23) || (sd->status.class >= 4001 + 7 && sd->status.class < 4023)) //スパノビと転生二次職はJobレベルの最高が70
		up_level += 20;
	...
	const char* command, const char* message)
{
	...
	if ((pl_sd = map_nick2sd(character)) != NULL) {
		...
		if (pc_isGM(sd) > pc_isGM(pl_sd)) {
			if (pl_s_class == 0)
				max_level -= 40;
			if ((pl_s_class == 23) || (pl_sd->status.class >= 4001 + 7 && pl_sd->status.class < 4023)) //スパノビと転生職はJobレベルの最高が70
				max_level += 20;
