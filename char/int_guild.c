		...
		for(i=0;i<MAX_GUILD;i++){
			if( g->member[i].account_id==account_id &&
				...
				if( guild_check_empty(g)==0 )
					mapif_guild_info(-1,g);// まだ人がいるのでデータ?信
				else
					inter_guild_save();	// 解散したので一?セーブ
				return 0;
			}
		}
	}
...
// ・エ?ーなら0(false)、そうでないなら1(true)をかえさなければならない
